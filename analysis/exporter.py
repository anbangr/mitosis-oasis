...
class ExperimentExporter:
    ...
    def export_all(self):
        ...
        log.info(f"Starting export from {self.db_path} to {self.out_dir}")
        ...
        for t in tables:
            ...
            if rows:
                self._write_csv(f"{t}.csv", rows)
                
        # 2. Export Computed EQ Metrics
        metrics_summary = self.metrics.compute_all_metrics()
        self._write_csv("computed_metrics.csv", [metrics_summary])
...
if __name__ == "__main__":
    ...
    exporter = ExperimentExporter(db_path, args.out)
    exporter.export_all()
