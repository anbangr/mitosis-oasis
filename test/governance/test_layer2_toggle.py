...
class TestLLMEnabledReturnsAdvisory:
    ...
    def test_all_clerks_return_dict_when_enabled(self, governance_db):
        ...
        clerks = [
            ...
            Regulator(
                governance_db, "did:mock:clerk-regulator", llm_enabled=True, llm=llm
            ),
            Codifier(
                governance_db, "did:mock:clerk-codifier", llm_enabled=True, llm=llm
            ),
        ]
        contexts = [
            {
...
class TestLayer1UnaffectedByLayer2:
    ...
    def test_layer1_same_with_and_without_llm(self, governance_db):
        ...
        conn.execute(
            "DELETE FROM message_log WHERE sender_did = 'did:mock:producer-1' AND msg_type = 'IDENTITY_ATTESTATION'"
        )
        conn.commit()
        conn.close()
